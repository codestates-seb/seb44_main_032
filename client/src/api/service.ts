...
export const getCommunityList = async (query: {
  ...
  try {
    if (search) {
      const response = await axios.get(`/community/search?query=${search}`);
      return response.data;
    }

    const response = await axios.get(
      `/community/${category || 'all'}?size=${pageSize || 10}&page=${
        page || 1
      }`,
    );
    return response.data;
...
export const deleteUser = async (userId?: number) => {
  try {
    ...
    const response = await axios.delete(`/user/delete/${userId || 1}`);
...
export const getUserInfo = async (userId?: number) => {
  try {
    const response = await axios.get(`/user/${userId || 1}`);
...
export const patchUserInfo = async (
  ...
  try {
    const response = await axios.patch(`/user/edit/${userId || 1}`, body);
